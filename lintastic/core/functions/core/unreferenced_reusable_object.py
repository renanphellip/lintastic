...
def unreferencedReusableObject(
    inputs: UnreferencedReusableObjectFunctionInputs,
) -> List[str]:
    ...
    if not _ref_exists(inputs.target_value, reusable_objects_location):
        return [f'No reference to {reusable_objects_location} was found ' f'in {inputs.context}.']
    return []
...
def _ref_exists(data: Any, reusable_objects_location: str) -> bool:
    if isinstance(data, dict):
        for key, value in data.items():
            if key == '$ref' and isinstance(value, str) and value == reusable_objects_location:
                return True
            if isinstance(value, dict) and _ref_exists(value, reusable_objects_location):
                return True
