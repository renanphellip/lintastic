...
class RefResolver:
    def __init__(self, file_reader_service = FileReaderService(), verbose = False, console = Console(highlight=False)):
    ...
    def __resolve_external_reference(self, file_name: str, base_path: str) -> Any:
        file_path = f'{base_path}/{file_name}'
        external_ref_content = self.file_reader_service.read_file(file_path, self.verbose)
        if self.verbose:
