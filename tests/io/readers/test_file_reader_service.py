...
@pytest.fixture
def file_reader_service(mock_console):
    return FileReaderService(mock_console)

def test_read_file_success(mocker, file_reader_service, mock_console):
...
def test_read_file_error(mocker, file_reader_service, mock_console):
    file_path = 'document.json'
    ...
    with pytest.raises(SystemExit):
        file_reader_service.read_file(file_path)
    
    mock_console.print.assert_called_once_with(expected_error_message)

def test_read_file_verbose(mocker, file_reader_service, mock_console):
    file_path = 'document.json'
    absolute_file_path = f'/absolute/path/to/{file_path}'
